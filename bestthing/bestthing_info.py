...
def get_standings_handler(message):
    return Concept.sorted_concepts()
...
while True:
    ...
    if add_concept_sckt in socks and socks[add_concept_sckt] == zmq.POLLIN:
        message = add_concept_sckt.recv()
        add_concept(message)
        add_concept_sckt.send('added')
    if judge_concepts_sckt in socks and socks[judge_concepts_sckt] == zmq.POLLIN:
        message = judge_concepts_sckt.recv()
        (c1, c2) = judge_concepts(message)
        if c1 is None:
            judge_concepts_sckt.send('No more or very less concepts to judge')
        else:
            if c1 == 'okay':
